    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(tabs: [
            Tab(icon: const Icon(Icons.flight_land), text: l10n.dspTabArrival),
            Tab(icon: const Icon(Icons.flight_takeoff), text: l10n.dspTabDeparture),
          ]),
        ),
...
String _flightLabel(Map<String, dynamic> m, String direction) {
  final f = (direction == 'arrival' ? m['arrivalFlight'] : m['departureFlight'])
      as Map<String, dynamic>?;
  if (f == null) return '';
  final no = f['flight_no'] as String? ?? '';
  final t = _hhmm(direction == 'arrival'
...
class _DispatchTab extends ConsumerWidget {
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(l10n.dspAutoDone(
                (r['assigned'] as num).toInt(), (r['unassigned'] as num).toInt())),
          ));
        }
  ...
  Widget _vanCard(BuildContext context, Map<String, dynamic> run, VoidCallback refresh) {
    final l10n = AppLocalizations.of(context)!;
    ...
    final title = [vehicle, driver ?? l10n.dspDriverUnset]
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              const Icon(Icons.directions_bus, size: 18, color: Color(0xFF0F7A6E)),
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  ...
                  PopupMenuItem(value: 'delete', child: Text(l10n.dspDeleteVan)),
                ],
              ),
            ]),
              ...
              Wrap(
                ...
                children: members.map((m) {
                  ...
                  return Chip(
                    ...
                    label: Text(label.isEmpty ? name : '$name  $label',
                        style: const TextStyle(fontSize: 12)),
                    onDeleted: () async {
                      await ApiClient.unassignFromRun(programId, run['id'] as String,
  ...
  // 미배차 인원을 특정 밴에 수동 배정
  Future<void> _pickVan(BuildContext context, List<Map<String, dynamic>> runs,
      Map<String, dynamic> person, VoidCallback refresh) async {
    final l10n = AppLocalizations.of(context)!;
    ...
    final runId = await showDialog<String>(
      ...
      builder: (_) => SimpleDialog(
        ...
        children: runs.map((run) {
          ...
          final cap = (run['capacity'] as num).toInt();
          final title = [run['vehicle'], run['driver_name']]
              .where((s) => s != null && '$s'.isNotEmpty)
              .join(' · ');
          return SimpleDialogOption(
            ...
            child: Row(children: [
              ...
              Text('$n/$cap', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
            ]),
          );
        }).toList(),
      ),
    ...
    try {
      await ApiClient.assignToRun(programId, runId,
  ...
  Future<void> _showVanDialog(BuildContext context, VoidCallback refresh,
    ...
    try {
      if (existing == null) {
        await ApiClient.createRun(programId, {'direction': direction, ...result});
      } else {
...
class _UnassignedCard extends StatelessWidget {
  ...
  const _UnassignedCard(
      {required this.direction, required this.people, required this.onTap});
    ...
    if (people.isEmpty) {
      return Container(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 8),
          Text(l10n.dspAllAssigned, style: TextStyle(color: Colors.green[800])),
        ]),
      );
    }
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(l10n.dspUnassignedCount(people.length),
              style: TextStyle(
                  fontSize: 12, fontWeight: FontWeight.w700, color: Colors.amber[900])),
            ...
            Row(children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(labelText: '${l10n.dspAirport} *'),
                ),
              ),
              const SizedBox(width: 10),
              SizedBox(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(labelText: '${l10n.dspCapacityLabel} *'),
                ),
              ),
            ]),
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () {
            ...
            if (airport.isEmpty || driver.isEmpty || cap == null || cap <= 0) return;
