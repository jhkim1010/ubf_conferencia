...
/// **두 값을 절대 합치지 않는다.**
///
///   [due]     우리에게 내는 돈. 참가비 + 투어 값 - 확정된 할인.
///   [extrasKnown] 참가자가 따로 쓸 돈. 투어 값에 안 든 것(061·062)과
///                 수양회 전후 숙박비.
///
...
class RegistrationCost {
  ...
  bool get isEmpty => due == 0 && extrasKnown == 0 && !extrasUnsure;
  ...
  static RegistrationCost of({
    ...
    final due = (tierFee ?? 0).toDouble() + optionsTotal - discount;
    ...
    return RegistrationCost(
      ...
      extrasKnown: _round(tour.known + (hotelEstimate ?? 0)),
      // 묵을 밤은 있는데 등급을 아직 안 골랐으면 그것도 "모르는 것"이다.
      extrasUnsure:
          tour.unknown.isNotEmpty || (hotelNights > 0 && hotelEstimate == null),
    );
  }
