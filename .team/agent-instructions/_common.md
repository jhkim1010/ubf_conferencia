...
**실패도 같은 의심을 받아야 합니다.** 측정 방법이 대상을 바꿔놓는 경우가 있습니다.

실제 사례: Flutter 웹 화면을 검증하며 텍스트를 읽으려고 접근성(semantics)을 켠 뒤
스크린샷을 찍었더니 입력칸 라벨이 전부 사라져 보였다. UI 결함으로 판단해 보고했으나,
접근성을 켜면 Flutter 가 캔버스 위에 투명 `<input>` DOM 오버레이를 깔기 때문에
스크린샷이 그것을 캡처한 것이었다. 접근성을 끄고 찍으니 라벨이 정상이었고,
애초에 접근성 트리에도 `textbox "지부명 *"` 처럼 라벨이 멀쩡히 들어 있었다.

- **Flutter 웹 스크린샷은 접근성을 끈 상태로 찍으십시오.** 텍스트 추출이 필요하면
  접근성을 켜서 트리를 읽되, 시각 확인은 끈 상태로 따로 찍습니다.
- 결함을 보고하기 전에 **다른 방법으로 한 번 더 확인**하십시오. 두 방법이 엇갈리면
  둘 중 하나가 도구 문제입니다.
