...
**웹으로 확인할 때는 `./serve-web.sh` 를 쓰십시오.** `flutter build web` 을 직접 돌리면
서비스 워커가 예전 번들을 캐시해, 코드를 고쳐 다시 빌드해도 브라우저가 옛 화면을
계속 보여줍니다. 실제로 이 함정에 빠져 한참을 헤맸습니다. 스크립트는
`--pwa-strategy=none` 으로 빌드합니다. **포트 8080 을 바꾸지 마십시오** — 구글 OAuth 가
origin(포트 포함) 단위로 검사해 다른 포트는 `400 origin_mismatch` 로 막힙니다.

**cmux 브라우저 페인은 Flutter 웹 앱의 최종 판정 수단이 아닙니다.** 새로 빌드한 앱이
`#/loading` 에서 멈추고 흰 화면만 나오는 현상을 겪었다. 서비스워커·캐시·구 빌드·서버·
토큰 만료·CanvasKit·로딩 지연을 모두 배제했고, 같은 `/auth/me` 요청이 그 페인의
콘솔에서는 200 을 반환했다. **사용자의 일반 Chrome 에서는 정상 동작했다.**

- 페인에서 안 뜬다고 곧바로 코드 결함으로 단정하지 마십시오.
- 화면이 안 뜨면 **사용자에게 직접 열어보게 요청**하는 것이 가장 빠른 판별법입니다.
  서버·웹 주소와 무엇을 봐야 하는지를 함께 알려주십시오.
- 페인은 탐색·조작·텍스트 추출에는 쓸 만합니다. 다만 "렌더링이 안 된다"는 결론은
  다른 브라우저로 교차 확인한 뒤에만 내리십시오.
