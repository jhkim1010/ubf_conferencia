...
import { pickLanguage, errorText } from './services/messages.js';
...
// 오류 문구는 **영어와 스페인어를 함께** 내보낸다 (055·070).
//
// 라우트는 한국어로 적는다 — 이 저장소의 관례이고, 109 곳을 고치면 그중
// 하나는 반드시 빠뜨린다. 대신 응답이 나가는 길목에서 갈아 끼운다.
//
// 기기 언어를 따르지 않는다. 오류는 혼자 읽고 끝나는 말이 아니라 옆 사람에게
// 보여 주며 묻는 말이라, 화면에 한 언어만 있으면 그 순간 쓸모가 없어진다.
// 번역표에 없는 말은 한국어 그대로 나간다.
app.use((req, res, next) => {
  const json = res.json.bind(res);
  res.json = (body) => {
    if (body && typeof body === 'object' && typeof body.error === 'string') {
      return json({ ...body, error: errorText(body.error) });
    }
    return json(body);
  };
  next();
